...
class Peer:
	def __init__(self, pid, neighbor):
		...
		server1 = server.Server(self.pid, self.neighbor)
		server1.start()
		#start pushupdate (details in pushupdate.py module)
		pushupdate1 = pushupdate.Pushupdate(self.pid, self.neighbor)
		pushupdate1.start()
		#start client (details in client.py module)
		client1 = client.Client(self.pid, self.neighbor)
		client1.start()
...
def main():
	...
	p1 = Peer(1, config.dict[topology]['p1'])
