...
class Peer:
	def __init__(self, pid, neighbor):
		...
		server8 = server.Server(self.pid, self.neighbor)
		server8.start()
		#start pushupdate (details in pushupdate.py module)
		pushupdate8 = pushupdate.Pushupdate(self.pid, self.neighbor)
		pushupdate8.start()
		#start client (details in client.py module)
		client8 = client.Client(self.pid, self.neighbor)
		client8.start()
...
def main():
	...
	p8 = Peer(8, config.dict[topology]['p8'])
